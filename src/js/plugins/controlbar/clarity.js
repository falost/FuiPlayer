...
export default class Clarity extends Base {
  ...
  initTextList () {
    this.option.list.forEach(item => {
      ...
      li.html(`<span>${item.name}</span> <span>${item.pixel}</span>`);
      ...
      this.option.width && li.attr('style', `width:${this.option.width}`);
    });
  }
  ...
  click (e) {
    ...
    if(elem.tagName === 'LI') {
      ...
      this.switchClarity(url).then(() => {
        ...
        this.$text.text(elem.getAttribute('title'));
  ...
  switchClarity (url) {
    ...
    console.log(this.option.duration)
    console.log('------ End------')
    this.loadOption = {
